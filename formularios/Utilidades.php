...
function getAcciones($encontrado,$dardebaja)
{
	if($encontrado){
		echo '<div class="accion">';
		echo '<button class="btn btn-success" type="submit" name="btn_accion" value="actualizar"><i class="fa fa-floppy-o"></i> Actualizar</button>';
		if($dardebaja)
			echo '<button class="btn btn-warning dardebaja" type="submit" name="btn_accion" value="dardebaja"><i class="fa fa-trash"></i> Dar de baja</button>';
		echo '<button class="btn btn-danger eliminar" type="submit" name="btn_accion" value="eliminar"><i class="fa fa-trash"></i>Eliminar</button>';
		echo '<button class="btn btn-info" type="submit" name="btn_accion" value="cancelar"><i class="fa fa-ban"></i> Cancelar</button>';
		echo '</div>';
	}
	else{
		echo '<button class="btn btn-success" type="submit" name="btn_accion" value="agregar"><i class="fa fa-floppy-o"></i> Agregar</button>';
	}
}

function getAccionesAdmin($isAdd, $isUpdate, $permisos){
 	if($isAdd)
 	{
 		if($permisos['r'])
...
function getFooter(){
	return '<footer>Soporte: dic.malo@gmail.com, ultima actualización 02/Nov./2020.</footer>';
}
?>